...
def train(args):
    ...
    device = 'cuda:' + str(args.ppo_device)
    # device = torch.device("cuda:0" if args.cuda else "cpu")
    print(f'use nosie model : {args.noise}')
    if args.noise:
        log_file = '-{}-{}-s{}-eta{}-var{}'.format(args.env_name, args.algo, args.seed, args.eta, args.myalpha)
        logger.configure(dir=f'{args.log_dir}/noise', format_strs=['csv', 'stdout'], log_suffix=log_file)
    else:
    ...
    if args.algo == 'ppo':
        ...
        actor_critic_fake = PPOnet(
            obs_shape,
            envs.action_space.n,
            base_kwargs={'hidden_size': args.hidden_size})
        # copy model
        actor_critic_state_dict = actor_critic.state_dict()
        actor_critic_fake.load_state_dict(actor_critic_state_dict)
    ...
    if args.algo == 'idaac':
        ...
        agent = algo.PPO(
            ...
            eta=args.eta,
            alpha=args.myalpha,
            use_para_var=args.use_para_var,
            noise=args.noise,
            lr=args.lr,
            eps=args.eps,
            max_grad_norm=args.max_grad_norm)
